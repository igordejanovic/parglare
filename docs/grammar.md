...
!!! note
    ...
    thus the tree will be expanding to the left. This example will collect all
    ...
    so use the first version as it is more efficient and parglare provides
    built-in actions for these common cases.
...
Previous section gives the overview of the basic BNF syntax. If you got use to
...
a [parsing error](./handling_errors.md) will be raised.
...
a list of subexpression evaluation results. This require you to use positional
...
`obj` action. Thus, for rules using named matches, default action is to create
object with attributes whose names are those of LHS of the assignments and
values are from RHS of the assignments (or boolean values for `bool`
assignments). Each object is an instance of corresponding dynamically created
Python class.
